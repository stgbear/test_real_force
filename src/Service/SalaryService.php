...
class SalaryService
{
	...
	public function getSalary(Employee $employee): Salary
	{
		...
		$carDeductionValue = $this->deductionService->getCarDeduction($employee);
	...
	{
		$grossSalary = $salary->getSalary();
		$bonusValue = $salary->getBonus();
		$taxValue = ceil(($grossSalary + $bonusValue) * $salary->getTax());
		$deductionValue = $salary->getDeduction();
